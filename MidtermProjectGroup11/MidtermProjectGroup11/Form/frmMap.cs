...
namespace MidtermProjectGroup11
{
    public partial class frm_Map : MaterialForm
    {
        ...
        {
            Execute(gamePlay, new Modify(), new GamePlayCommand(gamePlay, Enum.GamePlayAction.MoveForMainPlayer));
            btn_Endturn.Enabled = true;
            btn_Roll.Enabled = false;
            Execute(gamePlay, new Modify(), new GamePlayCommand(gamePlay, Enum.GamePlayAction.IsEndGame));
            if (isEndGame)
            {
                Form prompt = new Form();
                prompt.Width = 500;
                prompt.Height = 500;
                prompt.Text = "🎆🎆BOOOOOOOOOOOOM🎆🎆 You Survived :||";
                ...
                panel.BackgroundImage = new Bitmap(Path.Combine(Application.StartupPath, @"Images", "Extra", "win.jpg"));
